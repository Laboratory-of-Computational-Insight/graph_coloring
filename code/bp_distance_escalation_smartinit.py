"""
Same escalation ladder as bp_distance_escalation.py (plain BP -> reinforced
BP -> hyperparameter-diverse BP, OR-combined, cap=100 total attempts), but
every BP call is seeded with a "smart init": psi biased toward the
Bethe-Hessian spectral guess (bias_strength, see
belief_propagation_coloring.py's init_bias parameter) instead of pure noise.

Motivation: smart_init_diagnostic.py confirmed Bethe-Hessian's raw guess is
already meaningfully better than random (0.667) at c=4.3-4.69 (e.g. 0.509
distance at c=4.69), so biasing BP's init toward it is a real head start,
not biasing toward noise. This script measures whether that head start
actually reduces retries-to-success or raises success rate, on the exact
same c-band as the random-init baseline, for a direct comparison.
"""
import argparse
import itertools
import json
...
def _run_one(args):
    A, rho, damping, seed, init_bias = args
    belief, _ = bp_reinforced_coloring(A, rho=rho, damping=damping, max_iter=3000, seed=seed,
                                        init_bias=init_bias, bias_strength=BIAS_STRENGTH)
    colors = belief.argmax(axis=1)
    conf = count_conflicts(A, colors)
    return seed, colors, conf


def run_rung_parallel(A, tasks, pool, init_bias):
    args = [(A, rho, damping, seed, init_bias) for (rho, damping, seed) in tasks]
...
def solve_escalation(A, seed_base, cap_total, pool, init_bias):
    per_rung = cap_total // 3
    attempts_so_far = 0

    tasks = [(0.0, 0.5, seed_base * 100000 + i) for i in range(per_rung)]
    colors, conf, n_tried, success = run_rung_parallel(A, tasks, pool, init_bias)
...
def parse_args():
    ...
    p.add_argument("--c-values", type=float, nargs="+",
                    default=[4.3, 4.4, 4.42, 4.44, 4.46, 4.48, 4.5, 4.52, 4.54,
                             4.56, 4.58, 4.6, 4.62, 4.64, 4.66, 4.68, 4.69])
    ...
    p.add_argument("--out", default=f"{RESULTS_DIR}/bp_distance_escalation_smartinit.json")
    return p.parse_args()
    ...
    with mp.Pool(args.n_workers) as pool:
        for c in args.c_values:
            ...
            for i in range(args.n_inst):
                ...
                bh_guess = spectral_init_coloring(adj, variant="bethe_hessian")

                colors, method, attempts_used, success = solve_escalation(
                    A, seed_base=i, cap_total=args.cap, pool=pool, init_bias=bh_guess)
