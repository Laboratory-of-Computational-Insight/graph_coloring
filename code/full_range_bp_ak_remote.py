"""
Remote (elad-laca, 16 cores) half of the full c-range BP+alon_kahale test at
n=2000, BP up to 1000 restarts (parallelized across cores), max_iter=5000.
Handles the harder/slower end: c=16,18,20 and the phase transition c=4.69.
"""
import sys
import time
import json
...
if __name__ == "__main__":
    ...
    c_list = [float(x) for x in sys.argv[1:]] if len(sys.argv) > 1 else [16, 18, 20, 4.69]
    all_results = {}
    ...
    for c in c_list:
        ...
        for i in range(N_INST):
            ...
            if ak_ok:
                ...
                n_solved += 1
                print(f"  inst={i}: alon_kahale SUCCESS", flush=True)
                continue
            ...
            n_solved += bp_ok
            print(f"  inst={i}: bp {'SUCCESS' if bp_ok else 'FAIL'} (restarts={used})", flush=True)

        dt = time.time() - t0
        mean_r = np.mean(restarts_used) if restarts_used else 0
        max_r = max(restarts_used) if restarts_used else 0
        print(f"[REMOTE] c={c_disp}, n={N}: {n_solved}/{N_INST} ({100.0*n_solved/N_INST:.0f}%) "
              f"mean_bp_restarts={mean_r:.1f} max={max_r} time={dt:.0f}s", flush=True)
        all_results[str(c_disp)] = per_inst

    with open(f"{RESULTS_DIR}/full_range_bp_ak_remote.json", "w") as f:
        json.dump(all_results, f, indent=2)
    print("=== REMOTE PORTION COMPLETE ===")
