#!/usr/bin/env python3
"""
Evaluation matrix for multihead_gnn.py checkpoints, matching the style of
multistage_matrix_eval.py / pairwise_nn_matrix_eval.py: rows = c values,
columns = plain adjacency spectral (reference) + one column per
band-specialized MultiHeadGNNEmbedder checkpoint. Cross-checks every reported
...
# name -> (checkpoint filename, MultiHeadGNNEmbedder kwargs)
MANIFEST = {
...
def main():
    ...
    for name in models:
        header += name.rjust(max(14, len(name) + 2))
    print(header)
    ...
    for c in args.c_values:
        ...
        for _ in range(args.n_inst):
            ...
            for name, m in models.items():
                r = multihead_gnn_3color(adj, m, device="cpu")
                if r["success"]:
        ...
        for name in models:
            row += f"{row_counts[name]}/{args.n_inst}".rjust(max(14, len(name) + 2))
        print(row)
